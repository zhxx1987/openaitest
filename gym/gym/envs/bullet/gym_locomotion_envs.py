from gym.envs.bullet.scene_stadium import SinglePlayerStadiumScene
from gym.envs.bullet.env_bases import MJCFBaseBulletEnv ,URDFBaseBulletEnv
import numpy as np
import pybullet 
from gym.envs.bullet.robot_locomotors import Hopper, Walker2D, HalfCheetah, Ant, Humanoid, HumanoidFlagrun, HumanoidFlagrunHarder, Bike
...
#####################################################################################
class WheelBaseBulletEnv(URDFBaseBulletEnv):
    def __init__(self, robot, render=False):
        print("WalkerBase::__init__ start")
    ...
    def create_single_player_scene(self, bullet_client):
        self.stadium_scene = SinglePlayerStadiumScene(bullet_client, gravity=9.8, timestep=1.0/240., frame_skip=4)
        return self.stadium_scene
    ...
    def _reset(self):
        ...
        r = URDFBaseBulletEnv._reset(self)
        self._p.configureDebugVisualizer(pybullet.COV_ENABLE_RENDERING, 0)
    ...
    def _step(self, a):
        ...
        self._alive = float(self.robot.alive_bonus(state[0] + self.robot.initial_z, self.robot.body_rpy[
...
class BikeBulletEnv(WheelBaseBulletEnv):
	def __init__(self):
		self.robot = Bike()
		WheelBaseBulletEnv.__init__(self, self.robot)